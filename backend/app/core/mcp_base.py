...
class ToolCapability(BaseModel):
    ...
    supported_formats: List[str] = Field(default_factory=list, description="支持的输入格式")
...
class MCPToolBase(ABC):
    ...
    def __init__(self):
        self.name: str = self.__class__.__name__.lower().replace('mcp', '').replace('tool', '')
    ...
    async def prepare_execution(self, args: Dict[str, Any], context: ExecutionContext) -> bool:
        """准备执行环境"""
        # 验证参数
    ...
    def create_error_result(self, tool_name: str, error: str, execution_time: float = 0.0) -> ExecutionResult:
        """创建错误结果"""
